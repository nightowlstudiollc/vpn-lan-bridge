...
case "$OS" in
    Darwin)
        ifconfig | grep -E "^utun|^tun|^tap" | cut -d: -f1 | while read iface; do
...
if [[ -n "$TARGET_HOST" ]]; then
    ...
    echo ""
    echo "  Port check (24800 - Synergy):"
    if nc -z -w 2 "$TARGET_HOST" 24800 2>/dev/null; then
        echo "    ✓ Port 24800 is open"
    else
        echo "    ✗ Port 24800 is closed or unreachable"
    fi
else
